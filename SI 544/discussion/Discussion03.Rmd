...
For this part, do not worry about understanding the code. Focus on the concepts first, we will get to the code later in the semester.
...
### Exercise 1: filter by latest date (max date) and generate the total number of confirmed cases for each state
```{r}
the_latest_date = max(US_statelevel_cases$date)
```
...
```{r}
#you can also get the same data by filtering the data for the latest date. Try to write the code for it.
US_statelevel_cases %>%
  filter(date == max(date)) %>%
  group_by(administrative_area_level_2) %>%
  summarise(total_confirmed = confirmed)
```
...
```
2. That `summarize()` function generates a new dataframe, and
3. That when using `summarize()`, always give names to the new columns in the new dataframe
```{r}
...
State_median
#the result has a column with a weird name, why?
```
