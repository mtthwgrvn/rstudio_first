...
candy %>%
    ...
    # geom_parallel_slopes(se=FALSE)
  # Recall the model: 
  # `winpercent = 38.262	+ 18.273*chocolate + 8.567*sugarpercent`
    # If chocolate == FALSE:
...
candy %>%
    ...
    # geom_smooth(method='lm', se=FALSE) +
  # Recall the model:
  # `winpercent = 39.778 + 13.051*chocolate + 5.221*sugarpercent + 10.586*chocolate*sugarpercent`
    # If chocolate == FALSE:
...
## Questions
1. What should we do when we have coefficients that are not significant?
1.1 drop them from the equation?
1.2 drop them from the model?

## Takeaway message
1. We ideally would like to start by choosing a model.
2. Given any fitted model, we can make interpretations.
3. Two models can tell comparable or different stories.
...
```
### Questions 
0. Are there interaction terms?
1. Which factors could you drop from the model above?
2. Which factors should you keep?

Note, there is no definite answer here. The takeaway message is that you can now interpret  the regression table line by line.